...
int CursorPosCalculator::m_nBorderWidth = 10;
int CursorPosCalculator::m_nTitleHeight = 36;
...
void WidgetData::setMax(bool bMax)
{
    ...
    if(m_bMax != bMax)
    {
        ...
        //鼠标在哪个屏幕就用哪个屏幕来最大化
        int at_screen_index = BfQtUtils::PointAtScreenNum(QCursor::pos());
        QRect need_sceen_rect = BfQtUtils::ScreenAvailableRect(at_screen_index);
        m_pWidget->setGeometry(need_sceen_rect);
        }
        else
        {
			//还原
...
void FramelessHelper::MoveWidgetFromLarge(QPoint vector, QWidget* obj)
{
	//获得要移动的窗口（从widgetData中取，而不是直接用参数，保证窗口在无边框管理器中注册过）
	WidgetData* data = d->m_widgetDataHash.value(static_cast<QWidget*>(obj));
	if (data)
	{
        if(data->isMax()){
		...
		QWidget* wid = data->getWidget();
		QPoint _cur_point = wid->pos();
		wid->move(_cur_point + vector);
	}
}
