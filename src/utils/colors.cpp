...
QColor Colors::colorFromText(const QString &text)
{
    if(!text.startsWith("#") || text.length() != 7)
        return Qt::black;
    QString r = text.mid(1, 2);
    QString g = text.mid(3, 2);
    QString b = text.mid(5, 2);
    bool ok;
    return QColor(r.toInt(&ok, 16), g.toInt(&ok, 16), b.toInt(&ok, 16));
}


int Colors::QColorToInt(const QColor& color) {
