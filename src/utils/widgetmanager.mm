...
void WidgetManager::minimiseWindow(QWidget *widget)
{
#ifdef Q_OS_MAC
...
bool WidgetManager::isActive(QWidget *widget)
{
    const QWidget *temp = widget;
    while (temp->parentWidget())
        temp = temp->parentWidget();
    return temp->isActiveWindow() && !temp->isMinimized() && temp->isVisible();
}
